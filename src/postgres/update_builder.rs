...
use crate::postgres::{ExpressionBuilder, SetBuilder, SetFieldUpdate, SetValue, WhereBuilder};
use anyhow::anyhow;
...
impl UpdateBuilder {
    ...
    pub fn set(&mut self, values: Vec<SetFieldUpdate>) -> anyhow::Result<&mut Self> {
        ...
        let builder = SetBuilder::build(values);
        for value in builder.values {
            match value {
                SetValue::Query(select_builder) => {
                    self.values.append(&mut select_builder.get_values());
                }
                SetValue::Value(v) => {
                    self.values.push(v);
                }
...
pub mod test_update_builder {
    use crate::postgres::{ConditionBuilder, ConditionValue, Operator, SelectBuilder};
    ...
    //    #[tokio::test]
    //    async fn test_update_dollar_sequential_using_subquery_filter() {
    //        let select_builder = SelectBuilder::new(placeholder)
    //        let mut builder = UpdateBuilder::new(PlaceholderKind::DollarSequential);
    //        let set_ok_result = builder.table("users").set(vec![
    //            SetFieldUpdate {
    //                field: "name".to_string(),
